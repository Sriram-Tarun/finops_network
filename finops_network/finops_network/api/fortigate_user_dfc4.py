...
FORTIGATE_SSH_IP   = "45.198.61.167"
...
def _check_duplicate_username_vdom(username: str, vdom: str, exclude_name: str = None) -> bool:
    filters  = {"custom_dfc_4_username": username, "custom_virtual_domain": vdom}
    existing = frappe.db.get_value("DFC 4 User", filters, "name")
    if not existing:
        return False
...
def _check_duplicate_group_name_vdom(group_name: str, vdom: str, exclude_name: str = None) -> bool:
    ...
    existing = frappe.db.get_value("DFC 4 User Group", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
        return False
        ...
        for indicator in policy_indicators:
            if indicator in full_output:
                frappe.throw(
                    ...
                    f"FortiGate rejected the password.\n\nSSH output:\n{full_output}"
                )
...
def validate(doc, method):
    ...
    if doc.custom_dfc_4_username and " " in doc.custom_dfc_4_username:
        frappe.throw("Username should not contain spaces.")
    if doc.is_new():
        if _check_duplicate_username_vdom(doc.custom_dfc_4_username, vdom):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_4_username}</b>' already exists in VDOM '<b>{vdom}</b>'."
            )
    else:
        if _check_duplicate_username_vdom(doc.custom_dfc_4_username, vdom, exclude_name=doc.name):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_4_username}</b>' already exists in VDOM '<b>{vdom}</b>'."
            )
...
def on_save(doc, method):
    if getattr(doc.flags, "ignore_fortigate_sync", False):
        return
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_4_username
    try:
        plain_password = get_decrypted_password("DFC 4 User", doc.name, "password")
    except Exception:
    ...
    if not user_exists:
        ...
        payload = {
            "name":   fg_username, "type": "password",
        ...
        if response.status_code == 200:
            if response_json.get("status") != "success":
                frappe.throw(f"DFC 4 User creation failed: {response_json.get('error_msg', response.text)}")
...
def check_username_vdom_exists(username, vdom, exclude_name=None):
    filters  = {"custom_dfc_4_username": username, "custom_virtual_domain": vdom or "root"}
    existing = frappe.db.get_value("DFC 4 User", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
...
def create_fortigate_user(docname, password=None):
    doc         = frappe.get_doc("DFC 4 User", docname)
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_4_username
    try:
        plain_password = password or get_decrypted_password("DFC 4 User", docname, "password")
    except Exception:
    ...
    if response.status_code == 200:
        ...
        frappe.throw(f"Bad request (400): {response_json.get('error_msg', response.text)}")
    else:
...
def create_fortigate_user_group(docname):
    doc  = frappe.get_doc("DFC 4 User Group", docname)
    vdom = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    url  = f"{BASE_URL}/user/group?vdom={vdom}"
...
def update_fortigate_user_group(docname):
    ...
    if get_response.status_code == 404:
        frappe.throw(f"Group '<b>{doc.group_name}</b>' not found in FortiGate (VDOM: {vdom}).<br><br>Please click <b>Create Group in Fortigate</b> first.")
    ...
    if current_fortigate_type != fortigate_group_type:
        frappe.throw(f"FortiGate does not allow changing Group Type on an existing group via API.<br><br><b>Current type in FortiGate:</b> {current_fortigate_type}<br><b>Requested type in Portal:</b> {fortigate_group_type}")
        ...
        for vdom in vdom_list:
            ...
            for u in users:
                ...
                existing_name = frappe.db.get_value(
                    "DFC 4 User",
                    {"custom_dfc_4_username": fg_username, "custom_virtual_domain": vdom},
                    "name"
                )
                if existing_name:
                    doc = frappe.get_doc("DFC 4 User", existing_name)
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype":               "DFC 4 User",
                            "custom_dfc_4_username": fg_username,
                            "password":              "",
                        ...
                        frappe.log_error(f"Skipped user '{fg_username}' in VDOM '{vdom}': {str(e)}", "DFC4 User Sync Skip")
                        skipped += 1

        frappe.db.commit()
        return {"status": "success", "created": created, "updated": updated, "skipped": skipped, "vdoms_synced": vdom_list}

    except Exception as e:
        frappe.log_error(str(e)[:200], "Fortigate DFC4 User Sync Error")
        return {"status": "error", "message": str(e)}
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for g in groups:
                ...
                group_type = {"firewall": "Firewall", "fsso-service": "FSSO", "rsso": "RSSO", "guest": "Guest"}.get(raw_type, "Firewall")
                members    = g.get("member", [])
                existing   = frappe.db.get_value("DFC 4 User Group", {"group_name": group_name, "custom_virtual_domain": vdom}, "name")
                if existing:
                    doc = frappe.get_doc("DFC 4 User Group", existing)
                    ...
                    for m in members:
                        ...
                        if not uname:
                            continue
                        frappe_user_name = frappe.db.get_value("DFC 4 User", {"custom_dfc_4_username": uname, "custom_virtual_domain": vdom}, "name")
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype": "DFC 4 User Group", "group_name": group_name,
                        ...
                        frappe.log_error(f"Skipped group '{group_name}' in VDOM '{vdom}': {str(e)}", "DFC4 User Group Sync Skip")
                        skipped += 1
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC4 User Group Sync Error")
        return {"status": "error", "message": str(e)}
...
def search_by_user_group(search_term):
    results = frappe.db.sql("""
        SELECT name, custom_dfc_4_username, user_group, custom_virtual_domain, user_account_status
        FROM `tabDFC 4 User`
...
def rename_fortigate_user(docname, new_username, ticket_id=None, remarks=None):
    doc          = frappe.get_doc("DFC 4 User", docname)
    vdom         = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    old_username = doc.custom_dfc_4_username
    ...
    try:
        plain_password = get_decrypted_password("DFC 4 User", docname, "password")
    except Exception:
        plain_password = doc.password
    ...
    if del_resp.status_code not in (200, 404):
        frappe.throw(f"Failed to delete old user '{old_username}' in FortiGate (status {del_resp.status_code}): {del_resp.text}")

    create_url  = f"{BASE_URL}/user/local?vdom={vdom}"
    payload     = {"name": new_username, "type": "password", "passwd": plain_password or "", "status": "enable" if doc.user_account_status else "disable"}
    ...
    if create_resp.status_code == 500 and create_json.get("error") == -5:
        ...
        frappe.throw(f"Failed to create new user '{new_username}' in FortiGate (status {create_resp.status_code}): {create_resp.text}")
    ...
    doc.custom_dfc_4_username = new_username
    doc.flags.ignore_fortigate_sync = True
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 4 User", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment", "comment_type": "Info",
            "reference_doctype": "DFC 4 User", "reference_name": new_doc_name,
            "content": (
                f"<b>Action:</b> Rename User<br><b>Old Username:</b> {old_username}<br>"
                f"<b>New Username:</b> {new_username}<br><b>Ticket ID:</b> {ticket_id}"
            ...
            for field in ("groups", "srcaddr", "dstaddr"):
                for member in policy.get(field, []):
                    if member.get("name") == old_name:
                        ...
                        changed = True
            if changed:
                requests.put(f"{BASE_URL}/firewall/policy/{policyid}?vdom={vdom}", headers=HEADERS, json=policy, verify=False, timeout=30)
...
def rename_fortigate_user_group(docname, new_group_name, ticket_id=None, remarks=None):
    ...
    get_resp = requests.get(f"{BASE_URL}/user/group/{_safe_encode(old_group_name)}?vdom={vdom}", headers=HEADERS, verify=False, timeout=30)
    current_members = []
    ...
    create_resp = requests.post(
        f"{BASE_URL}/user/group?vdom={vdom}", headers=HEADERS,
    ...
    if create_resp.status_code == 500 and create_json.get("error") == -5:
        ...
        frappe.throw(f"Failed to create new group '{new_group_name}' in FortiGate (status {create_resp.status_code}): {create_resp.text}")

    updated_refs = _update_group_references(old_group_name, new_group_name, vdom)
    if updated_refs:
        frappe.msgprint(f"Updated group reference in: <b>{chr(44).join(updated_refs)}</b>", indicator="orange", alert=True)

    del_resp = requests.delete(f"{BASE_URL}/user/group/{_safe_encode(old_group_name)}?vdom={vdom}", headers=HEADERS, verify=False, timeout=30)
    if del_resp.status_code not in (200, 404):
        frappe.throw(f"Group '{new_group_name}' was created but deleting '{old_group_name}' failed (status {del_resp.status_code}): {del_resp.text}")

    new_doc_name = f"{new_group_name}|{vdom}"
    doc.flags.ignore_fortigate_sync = True
    doc.group_name = new_group_name
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 4 User Group", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment", "comment_type": "Info",
            "reference_doctype": "DFC 4 User Group", "reference_name": new_doc_name,
            "content": (
                f"<b>Action:</b> Rename Group<br><b>Old Group Name:</b> {old_group_name}<br>"
                f"<b>New Group Name:</b> {new_group_name}<br><b>Ticket ID:</b> {ticket_id}"
                + (f"<br><b>Remarks:</b> {remarks}" if remarks else "")
            )
        }).insert(ignore_permissions=True)
