...
def create_policy(docname):
    doc  = frappe.get_doc("DFC 4 Policy", docname)
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC4 Policy Create Error")
...
def update_policy(docname):
    ...
    for row in doc.custom_services:
        if row.service:
            svc_doc     = frappe.db.get_value("DFC 4 Service", row.service, "service_name", as_dict=True)
...
def get_interfaces(vdom="root"):
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC4 Interface Fetch Error")
        return []
    ...
    with ThreadPoolExecutor(max_workers=len(endpoints)) as executor:
        ...
        for future in as_completed(future_to_url):
            try:
                ...
                frappe.log_error(str(e)[:200], "Fortigate DFC4 Address Fetch Error")
...
def get_ip_pools(vdom="root"):
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC4 IP Pool Fetch Error")
        return []
...
def get_services(vdom="root"):
    ...
    for svc_name in services:
        ...
        if frappe.db.exists("DFC 4 Service", unique_name):
            continue
        if frappe.db.exists("DFC 4 Service", svc_name):
            continue
        try:
            svc_doc                       = frappe.new_doc("DFC 4 Service")
            ...
            frappe.log_error(str(e)[:200], f"Fortigate DFC4 Service Insert: {unique_name}")
...
def sync_firewall_services():
    ...
    if errors:
        frappe.log_error("\n".join(errors), "DFC4 Sync Firewall Services Errors")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                frappe.log_error(str(e)[:200], "FortiGate DFC4 Fetch VDOM Data Error")
    ...
    for vd in vdom_data_list:
        ...
        for pol in vd["policies"]:
            ...
            try:
                ...
                existing = frappe.db.exists("DFC 4 Policy", {
                ...
                doc = frappe.get_doc("DFC 4 Policy", existing) if existing else frappe.new_doc("DFC 4 Policy")
                ...
                frappe.log_error(
                    str(row_error)[:200],
                    f"DFC4 Policy Sync Error - VDOM: {vdom} Policy ID: {policy_id}"
                )
                skipped += 1
...
def validate_policy_uniqueness(docname, policy_name, vdom):
    ...
    if existing and existing != docname:
        return {
            "status": "duplicate",
            "message": "Policy {} already exists in VDOM {}".format(policy_name, vdom)
        }
    return {"status": "ok"}
...
def validate_policy(doc, method=None):
    ...
    if existing and existing != doc.name:
        frappe.throw(
            "Policy {} already exists in VDOM {}.".format(doc.policy_name, vdom),
