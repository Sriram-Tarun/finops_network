...
def create_policy(docname):
    doc  = frappe.get_doc("DFC 2 Policy", docname)
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Policy Create Error DFC2")
...
def update_policy(docname):
    ...
    for row in doc.custom_services:
        if row.service:
            svc_doc     = frappe.db.get_value("DFC 2 Service", row.service, "service_name", as_dict=True)
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Policy Update Error DFC2")
        frappe.throw(str(e))
...
def get_vdoms():
    try:
        ...
        frappe.log_error(str(e), "FortiGate Get VDOMs Error DFC2 Policy")
        return {"status": "error", "message": str(e)}
...
def get_interfaces(vdom="root"):
    try:
        ...
        for i in response.json().get("results", []):
            ...
            # 7.6.2 returns ALL interfaces regardless of vdom filter
            # so we filter manually by matching vdom field
        ...
        frappe.log_error(str(e), "Fortigate Interface Fetch Error DFC2 Policy")
        return []
    ...
    with ThreadPoolExecutor(max_workers=len(endpoints)) as executor:
        ...
        for future in as_completed(future_to_url):
            try:
                ...
                frappe.log_error(str(e), "Fortigate Address Fetch Error DFC2 Policy")
...
def get_ip_pools(vdom="root"):
    try:
        ...
        frappe.log_error(str(e), "Fortigate IP Pool Fetch Error DFC2 Policy")
        return []
...
def get_services(vdom="root"):
    ...
    for svc_name in services:
        ...
        if frappe.db.exists("DFC 2 Service", unique_name):
            continue
        if frappe.db.exists("DFC 2 Service", svc_name):
            continue
        try:
            svc_doc                       = frappe.new_doc("DFC 2 Service")
            ...
            frappe.log_error(str(e), f"Fortigate Service Insert DFC2: {unique_name}")
...
def sync_firewall_services():
    ...
    if errors:
        frappe.log_error("\n".join(errors), "Sync Firewall Services Errors DFC2")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                frappe.log_error(str(e), "FortiGate Fetch VDOM Data Error DFC2 Policy")
    ...
    for vd in vdom_data_list:
        ...
        for pol in vd["policies"]:
            ...
            try:
                ...
                existing = frappe.db.exists("DFC 2 Policy", {
                ...
                doc = frappe.get_doc("DFC 2 Policy", existing) if existing else frappe.new_doc("DFC 2 Policy")
                ...
                frappe.log_error(
                    str(row_error),
                    f"DFC2 Policy Sync Error - VDOM: {vdom} Policy ID: {policy_id}"
                )
                skipped += 1
