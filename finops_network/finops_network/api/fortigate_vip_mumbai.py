...
def delete_virtual_ip_on_fortigate(name, firewall_ip, api_token, vdom):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Mumbai VIP Delete Error")
        return False, str(e)


@frappe.whitelist()
def create_virtual_ip(docname):
    doc  = frappe.get_doc("DFC 1 Mumbai Virtual IP", docname)
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Mumbai VIP Create Error")
...
def update_virtual_ip(docname):
    ...
    if doc.port_forwarding and port_mapping_normalized == "many to many":
        deleted, msg = delete_virtual_ip_on_fortigate(doc.name1, FIREWALL_IP, API_TOKEN, vdom)
        if not deleted:
        ...
        try:
            ...
            frappe.log_error(str(e), "Mumbai VIP Update Recreate Error")
            return {"status": "error", "message": str(e)}
    else:
        ...
        try:
            ...
            frappe.log_error(str(e), "Mumbai VIP Update Error")
            return {"status": "error", "message": str(e)}
...
def sync_virtual_ips_from_fortigate():
    ...
    try:
        ...
        for vdom in vdoms:
            ...
            for v in vips:
                ...
                existing = frappe.db.exists(
                    "DFC 1 Mumbai Virtual IP",
                    {"name1": name, "custom_virtual_domain": vdom}
                )

                if existing:
                    doc = frappe.get_doc("DFC 1 Mumbai Virtual IP", existing)
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype":                  "DFC 1 Mumbai Virtual IP",
                            "name1":                    name,
        ...
        frappe.log_error(str(e), "Fortigate Mumbai VIP Sync Error")
        return {"status": "error", "message": str(e)}
...
def get_vdoms():
    ...
    try:
        ...
        frappe.log_error(str(e), "FortiGate Get VDOMs Error Mumbai VIP")
        return {"status": "error", "message": str(e)}
...
def rename_virtual_ip(docname, new_name, ticket_id=None, remarks=None):
    ...
    frappe.db.set_value("DFC 1 Mumbai Virtual IP", docname, "name1", new_name)
    frappe.db.commit()

    frappe.rename_doc("DFC 1 Mumbai Virtual IP", docname, new_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype":           "Comment",
            "comment_type":      "Info",
            "reference_doctype": "DFC 1 Mumbai Virtual IP",
