...
def get_vdom_list_for_service():
    try:
        ...
        frappe.log_error(str(e)[:200], "DFC32 VDOM Fetch Error")
...
def create_service_in_firewall(service_name, protocol, low, high, vdom=None):
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC32 Service Error")
        frappe.throw(str(e))
...
def sync_services_from_fortigate():
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            if response.status_code != 200:
                frappe.log_error(response.text, f"DFC32 Service Fetch Error - {vdom}")
                continue
            ...
            for svc in response.json().get("results", []):
                ...
                exists = frappe.db.get_value("DFC 3-2 Service", {"name": doc_name}, "name")
                ...
                if exists:
                    ...
                    try:
                        frappe.db.sql("""
                            INSERT INTO `tabDFC 3-2 Service`
                        ...
                        frappe.log_error(str(e)[:200], f"DFC32 Service Insert Failed: {doc_name}")
                        skipped += 1
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total": frappe.db.count("DFC 3-2 Service")
        }

    except Exception as e:
        frappe.log_error(str(e)[:200], "Fortigate DFC32 Service Sync Error")
        return {"status": "error", "message": str(e)}
