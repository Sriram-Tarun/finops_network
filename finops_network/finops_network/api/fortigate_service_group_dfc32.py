...
def get_vdom_list():
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "DFC32 SG VDOM Fetch Error")
        return {"status": "error", "message": str(e)}


@frappe.whitelist()
def create_service_group(docname):
    doc = frappe.get_doc("DFC 3-2 Service Group", docname)
    ...
    try:
        ...
        frappe.db.set_value("DFC 3-2 Service Group", docname, "custom_firewall_created", 1)
        frappe.db.commit()
        return f"Service Group '{doc.group_name.strip()}' created in FortiGate (VDOM: {vdom})"
    except Exception as e:
        frappe.log_error(str(e)[:200], "Fortigate DFC32 Service Group Error")
        frappe.throw(str(e))
...
def update_service_group(docname):
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC32 Update Service Group Error")
        frappe.throw(str(e))
...
def sync_service_groups_from_fortigate():
    ...
    try:
            ...
            for grp in response.json().get("results", []):
                ...
                if doc_name:
                    ...
                    if set(current_member_names) != set(member_names):
                        frappe.db.sql("""
                            UPDATE `tabDFC 3-2 Service Group`
                        ...
                        for idx, svc_name in enumerate(member_names, start=1):
                            frappe.db.sql(f"""
                                ...
                                VALUES (%s, %s, %s, %s, %s, %s)
                            """, (frappe.generate_hash(length=10), doc_name, "DFC 3-2 Service Group", "members", idx, svc_name))
                        updated += 1
                else:
                    new_doc_name = group_name
                    if frappe.db.exists("DFC 3-2 Service Group", new_doc_name):
                        new_doc_name = f"{group_name}-{vdom}"
                    frappe.db.sql("""
                        INSERT INTO `tabDFC 3-2 Service Group`
                    ...
                    for idx, svc_name in enumerate(member_names, start=1):
                        frappe.db.sql(f"""
                            ...
                            (name, parent, parenttype, parentfield, idx, service)
                            VALUES (%s, %s, %s, %s, %s, %s)
                        """, (frappe.generate_hash(length=10), new_doc_name, "DFC 3-2 Service Group", "members", idx, svc_name))
                    created += 1
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total": frappe.db.count("DFC 3-2 Service Group")
        }
    except Exception as e:
        frappe.log_error(str(e)[:200], "DFC32 Sync Service Group Error")
        return {"status": "error", "message": str(e)}
...
def get_services_by_vdom(vdom):
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "DFC32 Get Services By VDOM Error")
        return {"status": "error", "message": str(e)}
