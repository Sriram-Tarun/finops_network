...
def get_vdom_list():
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "DFC4 SG VDOM Fetch Error")
        return {"status": "error", "message": str(e)}


@frappe.whitelist()
def create_service_group(docname):
    doc = frappe.get_doc("DFC 4 Service Group", docname)
    ...
    payload = {"name": doc.group_name.strip(), "member": members}
    try:
...
def update_service_group(docname):
    ...
    payload = {"name": group_name, "member": members}
    try:
        ...
        if response.status_code == 404:
            frappe.throw(f"Service Group '{group_name}' not found in FortiGate (VDOM: {vdom}). Please create it first.")
        if result.get("status") != "success":
            frappe.throw(f"FortiGate Error: {str(result)}")
        return f"Service Group '{group_name}' updated successfully (VDOM: {vdom})"
    except Exception as e:
        frappe.log_error(str(e)[:200], "Fortigate DFC4 Update Service Group Error")
        frappe.throw(str(e))
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for grp in response.json().get("results", []):
                ...
                member_names = [m.get("name") for m in grp.get("member", []) if m.get("name")]
                ...
                if doc_name:
                    ...
                    if set(current_member_names) != set(member_names):
                        frappe.db.sql("""
                            UPDATE `tabDFC 4 Service Group`
                            SET modified = NOW(), modified_by = %s
                            WHERE name = %s
                        """, (frappe.session.user, doc_name))
                        frappe.db.sql(f"DELETE FROM `{child_table}` WHERE parent = %s", (doc_name,))
                        for idx, svc_name in enumerate(member_names, start=1):
                            frappe.db.sql(f"""
                                INSERT INTO `{child_table}`
                                (name, parent, parenttype, parentfield, idx, service)
                                VALUES (%s, %s, %s, %s, %s, %s)
                            """, (frappe.generate_hash(length=10), doc_name, "DFC 4 Service Group", "members", idx, svc_name))
                        updated += 1
                else:
                    new_doc_name = group_name
                    if frappe.db.exists("DFC 4 Service Group", new_doc_name):
                        new_doc_name = f"{group_name}-{vdom}"
                    frappe.db.sql("""
                        INSERT INTO `tabDFC 4 Service Group`
                        (name, group_name, custom_virtual_domain, custom_firewall_created,
                         owner, creation, modified, modified_by, docstatus)
                        VALUES (%s, %s, %s, 1, %s, NOW(), NOW(), %s, 0)
                    """, (new_doc_name, group_name, vdom, frappe.session.user, frappe.session.user))
                    for idx, svc_name in enumerate(member_names, start=1):
                        frappe.db.sql(f"""
                            INSERT INTO `{child_table}`
                            (name, parent, parenttype, parentfield, idx, service)
                            VALUES (%s, %s, %s, %s, %s, %s)
                        """, (frappe.generate_hash(length=10), new_doc_name, "DFC 4 Service Group", "members", idx, svc_name))
                    created += 1
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total": frappe.db.count("DFC 4 Service Group")
        }
    except Exception as e:
        frappe.log_error(str(e)[:200], "DFC4 Sync Service Group Error")
        return {"status": "error", "message": str(e)}
...
def get_services_by_vdom(vdom):
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "DFC4 Get Services By VDOM Error")
        return {"status": "error", "message": str(e)}
...
def search_services_by_vdom(doctype, txt, searchfield, start, page_len, filters):
    ...
    txt = (txt or "").lower()
    matched = [[s] for s in services if txt in s.lower() and s.strip().upper() != "ALL"]
    return matched[int(start): int(start) + int(page_len)]


@frappe.whitelist()
def validate_service_group_uniqueness(docname, group_name, vdom):
    vdom = vdom or "root"
    ...
    if existing and existing != docname:
        return {"status": "duplicate", "message": f"Service Group '{group_name}' already exists in VDOM '{vdom}'"}
    return {"status": "ok"}
...
def validate(doc, method=None):
    ...
    existing = frappe.db.get_value(
        "DFC 4 Service Group",
        {"group_name": doc.group_name, "custom_virtual_domain": vdom},
        "name"
    )
    if existing and existing != doc.name:
        frappe.throw(
            f"Service Group <b>{doc.group_name}</b> already exists in VDOM <b>{vdom}</b>.",
            title="Duplicate Service Group"
        )
