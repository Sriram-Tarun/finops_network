...
def create_policy(docname):
    doc  = frappe.get_doc("DFC 3-2 Policy", docname)
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC32 Policy Create Error")
...
def update_policy(docname):
    ...
    for row in doc.custom_services:
        if row.service:
            svc_doc     = frappe.db.get_value("DFC 3-2 Service", row.service, "service_name", as_dict=True)
...
def get_interfaces(vdom="root"):
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC32 Interface Fetch Error")
        return []
    ...
    with ThreadPoolExecutor(max_workers=len(endpoints)) as executor:
        ...
        for future in as_completed(future_to_url):
            try:
                ...
                frappe.log_error(str(e)[:200], "Fortigate DFC32 Address Fetch Error")
...
def get_ip_pools(vdom="root"):
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC32 IP Pool Fetch Error")
        return []
...
def get_services(vdom="root"):
    ...
    for svc_name in services:
        ...
        if frappe.db.exists("DFC 3-2 Service", unique_name):
            continue
        if frappe.db.exists("DFC 3-2 Service", svc_name):
            continue
        try:
            svc_doc                       = frappe.new_doc("DFC 3-2 Service")
            ...
            frappe.log_error(str(e)[:200], f"Fortigate DFC32 Service Insert: {unique_name}")
...
def sync_firewall_services():
    ...
    if errors:
        frappe.log_error("\n".join(errors), "DFC32 Sync Firewall Services Errors")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                frappe.log_error(str(e)[:200], "FortiGate DFC32 Fetch VDOM Data Error")
    ...
    for vd in vdom_data_list:
        ...
        for pol in vd["policies"]:
            ...
            try:
                ...
                existing = frappe.db.exists("DFC 3-2 Policy", {
                ...
                doc = frappe.get_doc("DFC 3-2 Policy", existing) if existing else frappe.new_doc("DFC 3-2 Policy")
                ...
                frappe.log_error(
                    str(row_error)[:200],
                    f"DFC32 Policy Sync Error - VDOM: {vdom} Policy ID: {policy_id}"
                )
                skipped += 1
