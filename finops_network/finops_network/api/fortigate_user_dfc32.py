...
def _check_duplicate_username_vdom(username: str, vdom: str, exclude_name: str = None) -> bool:
    """
    Returns True if a DFC 3-2 User already exists with this username+vdom combo.
    exclude_name: the current doc name to exclude from the check (for updates).
    """
    filters = {"custom_dfc_32_username": username, "custom_virtual_domain": vdom}
    existing = frappe.db.get_value("DFC 3-2 User", filters, "name")
    if not existing:
        return False
...
def _check_duplicate_group_name_vdom(group_name: str, vdom: str, exclude_name: str = None) -> bool:
    """
    Returns True if a DFC 3-2 User Group already exists with this group_name+vdom combo.
    exclude_name: the current doc name to exclude from the check (for updates).
    """
    filters = {"group_name": group_name, "custom_virtual_domain": vdom}
    existing = frappe.db.get_value("DFC 3-2 User Group", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
        return False
...
# -------------------------------------------------------
# Frappe Document Hook — fires on validate (before save) — DFC 3-2 User
# -------------------------------------------------------
def validate(doc, method):
    """
    Enforce composite uniqueness: custom_dfc_32_username + vdom must be unique.
    """
    if getattr(doc.flags, "ignore_fortigate_sync", False):
        return
    vdom = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    if doc.custom_dfc_32_username and " " in doc.custom_dfc_32_username:
        frappe.throw("Username should not contain spaces.")
    if doc.is_new():
        if _check_duplicate_username_vdom(doc.custom_dfc_32_username, vdom):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_32_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
    else:
        if _check_duplicate_username_vdom(doc.custom_dfc_32_username, vdom, exclude_name=doc.name):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_32_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
...
# -------------------------------------------------------
# Frappe Document Hook — validate for DFC 3-2 User Group
# -------------------------------------------------------
def validate_user_group(doc, method):
    """
    Enforce composite uniqueness: group_name + vdom must be unique for DFC 3-2 User Group.
    """
    if getattr(doc.flags, "ignore_fortigate_sync", False):
        return
    vdom = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    ...
    if doc.is_new():
        if _check_duplicate_group_name_vdom(doc.group_name, vdom):
            frappe.throw(
                f"User Group '<b>{doc.group_name}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same group name can only be created once per VDOM."
            )
    else:
...
# -------------------------------------------------------
# Frappe Document Hook — fires on every Save — DFC 3-2 User
# -------------------------------------------------------
def on_save(doc, method):
    if getattr(doc.flags, "ignore_fortigate_sync", False):
        return
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_32_username
    try:
        plain_password = get_decrypted_password("DFC 3-2 User", doc.name, "password")
    except Exception:
    ...
    if not user_exists:
        ...
        if response.status_code == 200:
            if response_json.get("status") != "success":
                frappe.throw(
                    f"DFC 3-2 User creation failed: "
            ...
            frappe.throw(
                f"Failed to create user '{fg_username}' in FortiGate "
                f"(status {response.status_code}): {response.text}"
            )
        frappe.msgprint(
            f"User '<b>{fg_username}</b>' created in FortiGate (VDOM: {vdom})",
        ...
        if response.status_code != 200:
            frappe.throw(
                f"Failed to update status for '{fg_username}' in FortiGate "
                f"(status {response.status_code}): {response.text}"
            )
        frappe.msgprint(
            f"User '<b>{fg_username}</b>' updated in FortiGate (VDOM: {vdom})",
...
# -------------------------------------------------------
# Frappe Document Hook — fires on Delete — DFC 3-2 User
# -------------------------------------------------------
def on_trash(doc, method):
    ...
    if response.status_code not in (200, 404):
        frappe.throw(
            f"Failed to delete '{fg_username}' from FortiGate "
            f"(status {response.status_code}): {response.text}"
        )
...
# -------------------------------------------------------
# -------------------------------------------------------
# Check username+vdom uniqueness (called from client — DFC 3-2 User)
# -------------------------------------------------------
@frappe.whitelist()
def check_username_vdom_exists(username, vdom, exclude_name=None):
    filters = {"custom_dfc_32_username": username, "custom_virtual_domain": vdom or "root"}
    existing = frappe.db.get_value("DFC 3-2 User", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
...
# -------------------------------------------------------
# Check group_name+vdom uniqueness (called from client — DFC 3-2 User Group)
# -------------------------------------------------------
@frappe.whitelist()
...
def create_fortigate_user(docname, password=None):
    doc         = frappe.get_doc("DFC 3-2 User", docname)
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_32_username
    try:
        plain_password = password or get_decrypted_password("DFC 3-2 User", docname, "password")
    except Exception:
...
def create_fortigate_user_group(docname):
    doc  = frappe.get_doc("DFC 3-2 User Group", docname)
    vdom = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    url  = f"{BASE_URL}/user/group?vdom={vdom}"
        ...
        for vdom in vdom_list:
            ...
            for u in users:
                ...
                existing_name = frappe.db.get_value(
                    "DFC 3-2 User",
                    {"custom_dfc_32_username": fg_username, "custom_virtual_domain": vdom},
                    "name"
                )

                if existing_name:
                    doc = frappe.get_doc("DFC 3-2 User", existing_name)
                    ...
                    doc.save(ignore_permissions=True)   # ? THIS WAS MISSING
                    updated += 1
                else:
                    try:
                        doc = frappe.get_doc({
                            "doctype":               "DFC 3-2 User",
                            "custom_dfc_32_username": fg_username,
                            "password":              "",
                        ...
                        frappe.log_error(
                            f"Skipped user '{fg_username}' in VDOM '{vdom}': {str(e)}",
                            "Fortigate User Sync Skip"
                        )
                        skipped += 1
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for g in groups:
                ...
                if existing:
                    doc = frappe.get_doc("DFC 3-2 User Group", existing)
                    ...
                    for m in members:
                        ...
                        frappe_user_name = frappe.db.get_value(
                            "DFC 3-2 User",
                            {"custom_dfc_32_username": uname, "custom_virtual_domain": vdom},
                            "name"
                        )
                        doc.append("members", {"username": frappe_user_name or uname})
                    doc.flags.ignore_fortigate_sync = True
                    doc.flags.ignore_validate_links = True
                    doc.save(ignore_permissions=True)   # ? THIS WAS MISSING
                    updated += 1
                    ...
                    try:
                        ...
                        for m in members:
                            ...
                            # Resolve to Frappe doc name (format: username|vdom)
                            doc.append("members", {"username": frappe_user_name or uname})
                        doc.flags.ignore_fortigate_sync = True
                        doc.insert(ignore_permissions=True, ignore_links=True)
                        created += 1
                        ...
                        frappe.log_error(
                            f"Skipped group '{group_name}' in VDOM '{vdom}': {str(e)}",
                            "Fortigate User Group Sync Skip"
                        )
                        skipped += 1
...
def search_by_user_group(search_term):
    results = frappe.db.sql("""
        SELECT name, custom_dfc_32_username, user_group, custom_virtual_domain, user_account_status
        FROM `tabDFC 3-2 User`
...
def rename_fortigate_user(docname, new_username, ticket_id=None, remarks=None):
    doc          = frappe.get_doc("DFC 3-2 User", docname)
    vdom         = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    old_username = doc.custom_dfc_32_username
    ...
    if _check_duplicate_username_vdom(new_username, vdom, exclude_name=docname):
        frappe.throw(
            f"User '<b>{new_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
            f"Please choose a different username."
        )

    try:
        plain_password = get_decrypted_password("DFC 3-2 User", docname, "password")
    except Exception:
        plain_password = doc.password
    ...
    # Step 2: Check if new username already exists in FortiGate
    ...
    # Step 5: Update username field and rename Frappe doc
    new_doc_name = f"{new_username}|{vdom}"
    doc.custom_dfc_32_username = new_username
    doc.flags.ignore_fortigate_sync = True
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 3-2 User", docname, new_doc_name, force=True)
    ...
    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment",
            "comment_type": "Info",
            "reference_doctype": "DFC 3-2 User",
...
# -------------------------------------------------------
# Helper: Find and update all references to a group name
# -------------------------------------------------------
def _update_group_references(old_name: str, new_name: str, vdom: str = "root"):
    updated = []

    pol_resp = requests.get(f"{BASE_URL}/firewall/policy?vdom={vdom}", headers=HEADERS, verify=False, timeout=30)
    if pol_resp.status_code == 200:
        for policy in pol_resp.json().get("results", []):
            policyid = policy.get("policyid")
            changed  = False
            for field in ("groups", "srcaddr", "dstaddr"):
                for member in policy.get(field, []):
                    if member.get("name") == old_name:
                        member["name"] = new_name
                        changed = True
            if changed:
                requests.put(
                    f"{BASE_URL}/firewall/policy/{policyid}?vdom={vdom}",
                    headers=HEADERS, json=policy, verify=False, timeout=30
                )
                updated.append(f"firewall/policy/{policyid}")

    ssl_resp = requests.get(f"{BASE_URL}/vpn.ssl/settings?vdom={vdom}", headers=HEADERS, verify=False, timeout=30)
    if ssl_resp.status_code == 200:
        ssl_data = ssl_resp.json().get("results", {})
        changed  = False
        for rule in ssl_data.get("authentication-rule", []):
            for member in rule.get("groups", []):
                if member.get("name") == old_name:
                    member["name"] = new_name
                    changed = True
        if changed:
            requests.put(
                f"{BASE_URL}/vpn.ssl/settings?vdom={vdom}",
                headers=HEADERS, json=ssl_data, verify=False, timeout=30
            )
            updated.append("vpn.ssl/settings")

    ztna_resp = requests.get(f"{BASE_URL}/firewall/access-proxy?vdom={vdom}", headers=HEADERS, verify=False, timeout=30)
    if ztna_resp.status_code == 200:
        for proxy in ztna_resp.json().get("results", []):
            proxy_name = proxy.get("name")
            changed    = False
            for rule in proxy.get("api-gateway", []):
                for member in rule.get("application", []):
                    if member.get("name") == old_name:
                        member["name"] = new_name
                        changed = True
            if changed:
                requests.put(
                    f"{BASE_URL}/firewall/access-proxy/{_safe_encode(proxy_name)}?vdom={vdom}",
                    headers=HEADERS, json=proxy, verify=False, timeout=30
                )
                updated.append(f"firewall/access-proxy/{proxy_name}")

    intf_resp = requests.get(f"{BASE_URL}/system/interface?vdom={vdom}", headers=HEADERS, verify=False, timeout=30)
    if intf_resp.status_code == 200:
        for intf in intf_resp.json().get("results", []):
            intf_name = intf.get("name")
            changed   = False
            for member in intf.get("captive-portal-exempt-groups", []):
                if member.get("name") == old_name:
                    member["name"] = new_name
                    changed = True
            if changed:
                requests.put(
                    f"{BASE_URL}/system/interface/{_safe_encode(intf_name)}?vdom={vdom}",
                    headers=HEADERS, json=intf, verify=False, timeout=30
                )
                updated.append(f"system/interface/{intf_name}")
...
def rename_fortigate_user_group(docname, new_group_name, ticket_id=None, remarks=None):
    ...
    # Check composite uniqueness for the new group name
    if _check_duplicate_group_name_vdom(new_group_name, vdom, exclude_name=docname):
        frappe.throw(
            f"User Group '<b>{new_group_name}</b>' already exists in VDOM '<b>{vdom}</b>'. "
            f"Please choose a different group name."
        )
    ...
    updated_refs = _update_group_references(old_group_name, new_group_name, vdom)

    if updated_refs:
        frappe.msgprint(
            f"Updated group reference in: <b>{', '.join(updated_refs)}</b>",
    ...
    if del_resp.status_code not in (200, 404):
        ...
        try:
            ...
            pass
        frappe.throw(
            f"Group '{new_group_name}' was created and references updated, but deleting "
            f"'{old_group_name}' failed (status {del_resp.status_code}).<br><br>"
            f"Please delete '{old_group_name}' manually from FortiGate.<br><br>"
            f"<b>FortiGate error:</b> {del_json.get('cli_error', del_resp.text)}"
        )

    # Update Frappe doc — use composite name group_name|vdom
    new_doc_name = f"{new_group_name}|{vdom}"
    doc.flags.ignore_fortigate_sync = True
    doc.group_name = new_group_name
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 3-2 User Group", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment",
            "comment_type": "Info",
            "reference_doctype": "DFC 3-2 User Group",
            "reference_name": new_doc_name,
            "content": (
                ...
                f"<b>Ticket ID:</b> {ticket_id}"
                + (f"<br><b>Updated refs:</b> {', '.join(updated_refs)}" if updated_refs else "")
                + (f"<br><b>Remarks:</b> {remarks}" if remarks else "")
            )
        }).insert(ignore_permissions=True)
