...
def _check_duplicate_username_vdom(username: str, vdom: str, exclude_name: str = None) -> bool:
    filters  = {"custom_dfc_1_bangalore_username": username, "custom_virtual_domain": vdom}
    existing = frappe.db.get_value("DFC 1 Bangalore User", filters, "name")
    if not existing:
        return False
...
def _check_duplicate_group_name_vdom(group_name: str, vdom: str, exclude_name: str = None) -> bool:
    ...
    existing = frappe.db.get_value("DFC 1 Bangalore User Group", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
        return False
...
# -------------------------------------------------------
# Frappe Document Hooks — DFC 1 Bangalore User
# -------------------------------------------------------
def validate(doc, method):
    ...
    if doc.custom_dfc_1_bangalore_username and " " in doc.custom_dfc_1_bangalore_username:
        frappe.throw("Username should not contain spaces.")

    if doc.is_new():
        if _check_duplicate_username_vdom(doc.custom_dfc_1_bangalore_username, vdom):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_1_bangalore_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
    else:
        if _check_duplicate_username_vdom(doc.custom_dfc_1_bangalore_username, vdom, exclude_name=doc.name):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_1_bangalore_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
...
def on_save(doc, method):
    ...
    vdom        = doc.custom_virtual_domain or "root"
    fg_username = doc.custom_dfc_1_bangalore_username

    try:
        plain_password = get_decrypted_password("DFC 1 Bangalore User", doc.name, "password")
    except Exception:
    ...
    if not user_exists:
        ...
        if response.status_code == 200:
            if response_json.get("status") != "success":
                frappe.throw(
                    f"DFC 1 Bangalore User creation failed: "
...
def check_username_vdom_exists(username, vdom, exclude_name=None):
    filters  = {"custom_dfc_1_bangalore_username": username, "custom_virtual_domain": vdom or "root"}
    existing = frappe.db.get_value("DFC 1 Bangalore User", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
...
def create_fortigate_user(docname, password=None):
    doc         = frappe.get_doc("DFC 1 Bangalore User", docname)
    vdom        = doc.custom_virtual_domain or "root"
    fg_username = doc.custom_dfc_1_bangalore_username
    try:
        plain_password = password or get_decrypted_password("DFC 1 Bangalore User", docname, "password")
    except Exception:
...
def sync_users_from_fortigate():
    try:
        ...
        for vdom in vdom_list:
            ...
            for u in users:
                ...
                existing_name = frappe.db.get_value(
                    "DFC 1 Bangalore User",
                    {"custom_dfc_1_bangalore_username": fg_username, "custom_virtual_domain": vdom},
                    "name"
                )

                if existing_name:
                    doc = frappe.get_doc("DFC 1 Bangalore User", existing_name)
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype":                        "DFC 1 Bangalore User",
                            "custom_dfc_1_bangalore_username": fg_username,
                            "password":                       "",
        ...
        frappe.log_error(str(e), "Fortigate Bangalore User Sync Error")
        return {"status": "error", "message": str(e)}


# -------------------------------------------------------
# Sync User Groups (placeholder — update after User Group doctype is ready)
# -------------------------------------------------------
@frappe.whitelist()
def sync_user_groups_from_fortigate():
    try:
        ...
        for vdom in vdom_list:
            ...
            for g in groups:
                ...
                existing = frappe.db.get_value(
                    "DFC 1 Bangalore User Group",
                    {"group_name": group_name, "custom_virtual_domain": vdom},
                    "name"
                )

                if existing:
                    doc = frappe.get_doc("DFC 1 Bangalore User Group", existing)
                    ...
                    for m in members:
                        ...
                        frappe_user_name = frappe.db.get_value(
                            "DFC 1 Bangalore User",
                            {"custom_dfc_1_bangalore_username": uname, "custom_virtual_domain": vdom},
                            "name"
                        )
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype":               "DFC 1 Bangalore User Group",
                        ...
                        frappe.log_error(
                            ...
                            "Bangalore User Group Sync Skip"
                        )
                        skipped += 1
        ...
        frappe.log_error(str(e), "Fortigate Bangalore User Group Sync Error")
        return {"status": "error", "message": str(e)}
...
def rename_fortigate_user(docname, new_username, ticket_id=None, remarks=None):
    doc          = frappe.get_doc("DFC 1 Bangalore User", docname)
    vdom         = doc.custom_virtual_domain or "root"
    old_username = doc.custom_dfc_1_bangalore_username
    ...
    try:
        plain_password = get_decrypted_password("DFC 1 Bangalore User", docname, "password")
    except Exception:
        plain_password = doc.password
    ...
    doc.custom_dfc_1_bangalore_username = new_username
    doc.flags.ignore_fortigate_sync     = True
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 1 Bangalore User", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype":            "Comment",
            "comment_type":       "Info",
            "reference_doctype":  "DFC 1 Bangalore User",
...
def search_by_user_group(search_term):
    results = frappe.db.sql("""
        SELECT name, custom_dfc_1_bangalore_username, user_group, custom_virtual_domain, user_account_status
        FROM `tabDFC 1 Bangalore User`
...
# -------------------------------------------------------
# User Group CRUD (placeholder — flesh out after doctype ready)
# -------------------------------------------------------
@frappe.whitelist()
def create_fortigate_user_group(docname):
    doc  = frappe.get_doc("DFC 1 Bangalore User Group", docname)
    vdom = doc.custom_virtual_domain or "root"
    url  = f"{BASE_URL}/user/group?vdom={vdom}"
...
def rename_fortigate_user_group(docname, new_group_name, ticket_id=None, remarks=None):
    ...
    if del_resp.status_code not in (200, 404):
        frappe.throw(
            f"Group '{new_group_name}' was created but deleting '{old_group_name}' failed "
            f"(status {del_resp.status_code}): {del_resp.text}"
        )
    ...
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 1 Bangalore User Group", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype":           "Comment",
            "comment_type":      "Info",
            "reference_doctype": "DFC 1 Bangalore User Group",
            "reference_name":    new_doc_name,
            "content": (
