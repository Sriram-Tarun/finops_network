...
def create_address(docname):
    doc  = frappe.get_doc("DFC 1 Bangalore Address", docname)
    vdom = doc.custom_virtual_domain or "root"
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Bangalore Address Create Error")
        return {"status": "error", "message": str(e)}
...
def update_address(docname):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Bangalore Address Update Error")
        return {"status": "error", "message": str(e)}
...
def sync_addresses_from_fortigate():
    ...
    try:
        ...
        vdom_list = (
            ...
            if vdom_response.status_code == 200 else ["root"]
        )
        ...
        for vdom in vdom_list:
            try:
                ...
                frappe.log_error(str(e), f"FortiGate Bangalore Address Sync Error - VDOM: {vdom}")
                continue
            ...
            for addr in data.get("results", []):
                ...
                if existing:
                    doc = frappe.get_doc("DFC 1 Bangalore Address", existing)
                    ...
                    new_doc = {
                        "doctype":               "DFC 1 Bangalore Address",
                        "name1":                 name,
                        "type":                  frappe_type,
        ...
        frappe.log_error(str(e), "FortiGate Bangalore Address Sync Error")
        return {"status": "error", "message": str(e)}
