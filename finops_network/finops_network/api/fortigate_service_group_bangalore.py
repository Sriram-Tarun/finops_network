...
def get_vdom_list():
    try:
        ...
        frappe.log_error(str(e), "Bangalore VDOM Fetch Error")
        return {"status": "error", "message": str(e)}


@frappe.whitelist()
def create_service_group(docname):
    doc  = frappe.get_doc("DFC 1 Bangalore Service Group", docname)
    ...
    try:
        ...
        frappe.db.set_value("DFC 1 Bangalore Service Group", docname, "custom_firewall_created", 1)
        frappe.db.commit()

        return f"Service Group '{doc.group_name.strip()}' created in FortiGate (VDOM: {vdom})"

    except Exception as e:
        frappe.log_error(str(e), "Fortigate Bangalore Service Group Create Error")
        frappe.throw(str(e))
...
def update_service_group(docname):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Bangalore Update Service Group Error")
        frappe.throw(str(e))
...
def sync_service_groups_from_fortigate():
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for grp in response.json().get("results", []):
                ...
                if doc_name:
                    ...
                    if changed:
                        frappe.db.sql("""
                            UPDATE `tabDFC 1 Bangalore Service Group`
                    ...
                    if frappe.db.exists("DFC 1 Bangalore Service Group", new_doc_name):
                        new_doc_name = f"{group_name}-{vdom}"

                    frappe.db.sql("""
                        INSERT INTO `tabDFC 1 Bangalore Service Group`
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total":   frappe.db.count("DFC 1 Bangalore Service Group")
        }

    except Exception as e:
        frappe.log_error(str(e), "Bangalore Sync Service Group Error")
        return {"status": "error", "message": str(e)}
...
def get_services_by_vdom(vdom):
    ...
    try:
        ...
        frappe.log_error(str(e), "Bangalore Get Services By VDOM Error")
        return {"status": "error", "message": str(e)}
