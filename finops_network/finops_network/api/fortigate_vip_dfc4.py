...
def delete_virtual_ip_on_fortigate(name, vdom):
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC4 VIP Delete Error")
        return False, str(e)
...
def get_vdoms():
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "FortiGate DFC4 VIP Get VDOMs Error")
        return {"status": "error", "message": str(e)}


@frappe.whitelist()
def create_virtual_ip(docname):
    doc  = frappe.get_doc("DFC 4 Virtual IP", docname)
    ...
    try:
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC4 VIP Create Error")
        return {"status": "error", "message": str(e)}
...
def update_virtual_ip(docname):
    ...
    if doc.port_forwarding and port_mapping_normalized == "many to many":
        ...
        try:
            response = requests.post(create_url, headers=headers, json=payload, verify=False, timeout=20)
            if response.status_code == 200:
                return {"status": "success", "message": f"VIP updated (recreated) in VDOM '{vdom}'"}
            return {"status": "error", "message": response.text}
        except Exception as e:
            frappe.log_error(str(e)[:200], "DFC4 VIP Update Recreate Error")
            return {"status": "error", "message": str(e)}
    else:
        update_url = f"https://{FIREWALL_IP}/api/v2/cmdb/firewall/vip/{doc.name1}?vdom={vdom}"
        try:
            response = requests.put(update_url, headers=headers, json=payload, verify=False, timeout=20)
            if response.status_code == 200:
                return {"status": "success", "message": f"VIP updated successfully in VDOM '{vdom}'"}
            return {"status": "error", "message": response.text}
        except Exception as e:
            frappe.log_error(str(e)[:200], "DFC4 VIP Update Error")
            return {"status": "error", "message": str(e)}
    ...
    try:
        ...
        for vdom in vdoms:
            ...
            for v in vips:
                ...
                if existing:
                    doc = frappe.get_doc("DFC 4 Virtual IP", existing)
        ...
        frappe.log_error(str(e)[:200], "Fortigate DFC4 VIP Sync Error")
        return {"status": "error", "message": str(e)}
...
def rename_virtual_ip(docname, new_name, ticket_id=None, remarks=None):
    ...
    frappe.db.set_value("DFC 4 Virtual IP", docname, "name1", new_name)
    frappe.db.commit()
    frappe.rename_doc("DFC 4 Virtual IP", docname, new_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment", "comment_type": "Info",
            "reference_doctype": "DFC 4 Virtual IP", "reference_name": new_name,
            "content": (
                f"<b>Action:</b> Rename Virtual IP<br><b>Old Name:</b> {old_name}<br>"
                f"<b>New Name:</b> {new_name}<br><b>Ticket ID:</b> {ticket_id}"
                + (f"<br><b>Remarks:</b> {remarks}" if remarks else "")
            )
        }).insert(ignore_permissions=True)

    return {"status": "success", "message": f"Virtual IP '<b>{old_name}</b>' renamed to '<b>{new_name}</b>' successfully."}
...
def validate_vip_uniqueness(docname, name1, vdom):
    ...
    if existing and existing != docname:
        return {"status": "duplicate", "message": f"Virtual IP '{name1}' already exists in VDOM '{vdom}'"}
    return {"status": "ok"}
