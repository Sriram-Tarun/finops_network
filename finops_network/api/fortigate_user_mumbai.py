...
# Configuration
# -------------------------------------------------------
FORTIGATE_IP       = "103.216.92.148"
API_TOKEN          = "G8mqG51fQbgN5tk1Nnmy4mGtHwhxc0"

FORTIGATE_SSH_IP   = "103.216.92.148"
...
def _check_duplicate_username_vdom(username: str, vdom: str, exclude_name: str = None) -> bool:
    filters  = {"custom_dfc_1_mumbai_username": username, "custom_virtual_domain": vdom}
    existing = frappe.db.get_value("DFC 1 Mumbai User", filters, "name")
    if not existing:
        return False
...
def _check_duplicate_group_name_vdom(group_name: str, vdom: str, exclude_name: str = None) -> bool:
    ...
    existing = frappe.db.get_value("DFC 1 Mumbai User Group", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
        return False
...
# -------------------------------------------------------
# Frappe Document Hook — validate — DFC 1 Mumbai User
# -------------------------------------------------------
def validate(doc, method):
    ...
    if doc.custom_dfc_1_mumbai_username and " " in doc.custom_dfc_1_mumbai_username:
        frappe.throw("Username should not contain spaces.")

    if doc.is_new():
        if _check_duplicate_username_vdom(doc.custom_dfc_1_mumbai_username, vdom):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_1_mumbai_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
    else:
        if _check_duplicate_username_vdom(doc.custom_dfc_1_mumbai_username, vdom, exclude_name=doc.name):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_1_mumbai_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
...
# -------------------------------------------------------
# Frappe Document Hook — validate — DFC 1 Mumbai User Group
# -------------------------------------------------------
...
# -------------------------------------------------------
# Frappe Document Hook — on_save — DFC 1 Mumbai User
# -------------------------------------------------------
def on_save(doc, method):
    ...
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_1_mumbai_username

    try:
        plain_password = get_decrypted_password("DFC 1 Mumbai User", doc.name, "password")
    except Exception:
    ...
    if not user_exists:
        ...
        if response.status_code == 200:
            if response_json.get("status") != "success":
                frappe.throw(
                    f"DFC 1 Mumbai User creation failed: "
...
# -------------------------------------------------------
# Frappe Document Hook — on_trash — DFC 1 Mumbai User
# -------------------------------------------------------
...
def check_username_vdom_exists(username, vdom, exclude_name=None):
    filters  = {"custom_dfc_1_mumbai_username": username, "custom_virtual_domain": vdom or "root"}
    existing = frappe.db.get_value("DFC 1 Mumbai User", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
...
def get_vdoms():
    ...
    try:
        ...
        frappe.log_error(str(e), "FortiGate Get VDOMs Error Mumbai User")
...
def create_fortigate_user(docname, password=None):
    doc         = frappe.get_doc("DFC 1 Mumbai User", docname)
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_1_mumbai_username
    try:
        plain_password = password or get_decrypted_password("DFC 1 Mumbai User", docname, "password")
    except Exception:
...
def create_fortigate_user_group(docname):
    doc  = frappe.get_doc("DFC 1 Mumbai User Group", docname)
    vdom = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    url  = f"{BASE_URL}/user/group?vdom={vdom}"
...
def sync_users_from_fortigate():
    try:
        ...
        for vdom in vdom_list:
            ...
            for u in users:
                ...
                existing_name = frappe.db.get_value(
                    "DFC 1 Mumbai User",
                    {"custom_dfc_1_mumbai_username": fg_username, "custom_virtual_domain": vdom},
                    "name"
                )

                if existing_name:
                    doc = frappe.get_doc("DFC 1 Mumbai User", existing_name)
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype":                      "DFC 1 Mumbai User",
                            "custom_dfc_1_mumbai_username": fg_username,
                            "password":                     "",
                        ...
                        frappe.log_error(
                            ...
                            "Fortigate Mumbai User Sync Skip"
                        )
                        skipped += 1
        ...
        frappe.log_error(str(e), "Fortigate Mumbai User Sync Error")
        return {"status": "error", "message": str(e)}
...
def sync_user_groups_from_fortigate():
    try:
        ...
        for vdom in vdom_list:
            ...
            for g in groups:
                ...
                existing = frappe.db.get_value(
                    "DFC 1 Mumbai User Group",
                    {"group_name": group_name, "custom_virtual_domain": vdom},
                    "name"
                )

                if existing:
                    doc = frappe.get_doc("DFC 1 Mumbai User Group", existing)
                    ...
                    for m in members:
                        ...
                        frappe_user_name = frappe.db.get_value(
                            "DFC 1 Mumbai User",
                            {"custom_dfc_1_mumbai_username": uname, "custom_virtual_domain": vdom},
                            "name"
                        )
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype":               "DFC 1 Mumbai User Group",
                        ...
                        frappe.log_error(
                            ...
                            "Fortigate Mumbai User Group Sync Skip"
                        )
                        skipped += 1
        ...
        frappe.log_error(str(e), "Fortigate Mumbai User Group Sync Error")
        return {"status": "error", "message": str(e)}
...
def search_by_user_group(search_term):
    results = frappe.db.sql("""
        SELECT name, custom_dfc_1_mumbai_username, user_group, custom_virtual_domain, user_account_status
        FROM `tabDFC 1 Mumbai User`
...
def rename_fortigate_user(docname, new_username, ticket_id=None, remarks=None):
    doc          = frappe.get_doc("DFC 1 Mumbai User", docname)
    vdom         = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    old_username = doc.custom_dfc_1_mumbai_username
    ...
    try:
        plain_password = get_decrypted_password("DFC 1 Mumbai User", docname, "password")
    except Exception:
        plain_password = doc.password
    ...
    doc.custom_dfc_1_mumbai_username = new_username
    doc.flags.ignore_fortigate_sync  = True
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 1 Mumbai User", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment",
            "comment_type": "Info",
            "reference_doctype": "DFC 1 Mumbai User",
...
# -------------------------------------------------------
# Helper: Update group references in FortiGate policies
# -------------------------------------------------------
...
def rename_fortigate_user_group(docname, new_group_name, ticket_id=None, remarks=None):
    ...
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 1 Mumbai User Group", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment",
            "comment_type": "Info",
            "reference_doctype": "DFC 1 Mumbai User Group",
            "reference_name": new_doc_name,
            "content": (
