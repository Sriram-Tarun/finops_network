...
def get_vdom_list():
    try:
        ...
        frappe.log_error(str(e), "VDOM Fetch Error DFC1 Service Group")
...
def create_service_group(docname):
    doc  = frappe.get_doc("DFC 1 Service Group", docname)
    ...
    try:
        ...
        frappe.db.set_value("DFC 1 Service Group", docname, "custom_firewall_created", 1)
        frappe.db.commit()

        return f"Service Group '{doc.group_name.strip()}' created in FortiGate (VDOM: {vdom})"

    except Exception as e:
        frappe.log_error(str(e), "Fortigate Service Group Error DFC1")
        frappe.throw(str(e))
...
def update_service_group(docname):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Update Service Group Error DFC1")
        frappe.throw(str(e))
...
def sync_service_groups_from_fortigate():
    headers     = get_headers()
    child_table = "tabService Group Member DFC 1"
    created     = 0
    updated     = 0
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for grp in response.json().get("results", []):
                ...
                if doc_name:
                    ...
                    if frappe.db.exists("DFC 1 Service Group", new_doc_name):
                        new_doc_name = f"{group_name}-{vdom}"

                    frappe.db.sql("""
                        INSERT INTO `tabDFC 1 Service Group`
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total":   frappe.db.count("DFC 1 Service Group")
        }

    except Exception as e:
        frappe.log_error(str(e), "Sync Service Group Error DFC1")
        return {"status": "error", "message": str(e)}
...
def get_services_by_vdom(vdom):
    ...
    try:
        ...
        frappe.log_error(str(e), "Get Services By VDOM Error DFC1")
        return {"status": "error", "message": str(e)}
