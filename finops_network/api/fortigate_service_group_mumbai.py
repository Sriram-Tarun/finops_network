...
def get_vdom_list():
    try:
        ...
        frappe.log_error(str(e), "VDOM Fetch Error Mumbai Service Group")
...
def create_service_group(docname):
    doc  = frappe.get_doc("DFC 1 Mumbai Service Group", docname)
    ...
    try:
        ...
        frappe.db.set_value("DFC 1 Mumbai Service Group", docname, "custom_firewall_created", 1)
        frappe.db.commit()

        return f"Service Group '{doc.group_name.strip()}' created in FortiGate (VDOM: {vdom})"

    except Exception as e:
        frappe.log_error(str(e), "Fortigate Service Group Error Mumbai")
        frappe.throw(str(e))
...
def update_service_group(docname):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Update Service Group Error Mumbai")
        frappe.throw(str(e))


# ---------------------------------------------
# SYNC SERVICE GROUPS FROM FORTIGATE
# ---------------------------------------------
@frappe.whitelist()
def sync_service_groups_from_fortigate():
    headers     = get_headers()
    child_table = "tabService Group Member DFC 1 Mumbai"
    created     = 0
    updated     = 0
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for grp in response.json().get("results", []):
                ...
                if doc_name:
                    ...
                    if changed:
                        frappe.db.sql("""
                            UPDATE `tabDFC 1 Mumbai Service Group`
                    ...
                    if frappe.db.exists("DFC 1 Mumbai Service Group", new_doc_name):
                        new_doc_name = f"{group_name}-{vdom}"

                    frappe.db.sql("""
                        INSERT INTO `tabDFC 1 Mumbai Service Group`
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total":   frappe.db.count("DFC 1 Mumbai Service Group")
        }

    except Exception as e:
        frappe.log_error(str(e), "Sync Service Group Error Mumbai")
        return {"status": "error", "message": str(e)}


# ---------------------------------------------
# GET SERVICES BY VDOM
# ---------------------------------------------
@frappe.whitelist()
def get_services_by_vdom(vdom):
    ...
    try:
        ...
        frappe.log_error(str(e), "Get Services By VDOM Error Mumbai")
        return {"status": "error", "message": str(e)}
