...
def create_policy(docname):
    doc  = frappe.get_doc("DFC 1 Bangalore Policy", docname)
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Bangalore Policy Create Error")
...
def update_policy(docname):
    ...
    for row in doc.custom_services:
        if row.service:
            svc_doc     = frappe.db.get_value("DFC 1 Bangalore Service", row.service, "service_name", as_dict=True)
...
def get_interfaces(vdom="root"):
    try:
        ...
        frappe.log_error(str(e), "Fortigate Bangalore Interface Fetch Error")
        return []
    ...
    with ThreadPoolExecutor(max_workers=len(endpoints)) as executor:
        ...
        for future in as_completed(future_to_url):
            try:
                ...
                frappe.log_error(str(e), "Fortigate Bangalore Address Fetch Error")
...
def get_ip_pools(vdom="root"):
    try:
        ...
        frappe.log_error(str(e), "Fortigate Bangalore IP Pool Fetch Error")
        return []
...
def get_services(vdom="root"):
    ...
    for svc_name in services:
        ...
        if frappe.db.exists("DFC 1 Bangalore Service", unique_name):
            continue
        if frappe.db.exists("DFC 1 Bangalore Service", svc_name):
            continue
        try:
            svc_doc                       = frappe.new_doc("DFC 1 Bangalore Service")
            ...
            frappe.log_error(str(e), f"Fortigate Bangalore Service Insert: {unique_name}")
...
def sync_firewall_services():
    ...
    if errors:
        frappe.log_error("\n".join(errors), "Sync Bangalore Firewall Services Errors")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                frappe.log_error(str(e), "FortiGate Bangalore Fetch VDOM Data Error")
    ...
    for vd in vdom_data_list:
        ...
        for pol in vd["policies"]:
            ...
            try:
                ...
                existing = frappe.db.exists("DFC 1 Bangalore Policy", {
                ...
                doc = frappe.get_doc("DFC 1 Bangalore Policy", existing) if existing else frappe.new_doc("DFC 1 Bangalore Policy")
                ...
                frappe.log_error(
                    str(row_error),
                    f"Bangalore Policy Sync Error - VDOM: {vdom} Policy ID: {policy_id}"
                )
                skipped += 1
