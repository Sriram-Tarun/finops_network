...
def _check_duplicate_username_vdom(username: str, vdom: str, exclude_name: str = None) -> bool:
    filters  = {"custom_dfc_1_username": username, "custom_virtual_domain": vdom}
    existing = frappe.db.get_value("DFC 1 User", filters, "name")
    if not existing:
        return False
...
def _check_duplicate_group_name_vdom(group_name: str, vdom: str, exclude_name: str = None) -> bool:
    ...
    existing = frappe.db.get_value("DFC 1 User Group", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
        return False
...
# -------------------------------------------------------
# Frappe Document Hook — validate — DFC 1 User
# -------------------------------------------------------
def validate(doc, method):
    ...
    if doc.custom_dfc_1_username and " " in doc.custom_dfc_1_username:
        frappe.throw("Username should not contain spaces.")

    if doc.is_new():
        if _check_duplicate_username_vdom(doc.custom_dfc_1_username, vdom):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_1_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
    else:
        if _check_duplicate_username_vdom(doc.custom_dfc_1_username, vdom, exclude_name=doc.name):
            frappe.throw(
                f"User '<b>{doc.custom_dfc_1_username}</b>' already exists in VDOM '<b>{vdom}</b>'. "
                f"The same username can only be created once per VDOM."
            )
...
# -------------------------------------------------------
# Frappe Document Hook — validate — DFC 1 User Group
# -------------------------------------------------------
...
# -------------------------------------------------------
# Frappe Document Hook — on_save — DFC 1 User
# -------------------------------------------------------
def on_save(doc, method):
    ...
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_1_username

    try:
        plain_password = get_decrypted_password("DFC 1 User", doc.name, "password")
    except Exception:
...
# -------------------------------------------------------
# Frappe Document Hook — on_trash — DFC 1 User
# -------------------------------------------------------
...
# -------------------------------------------------------
# Uniqueness checks (called from client)
# -------------------------------------------------------
@frappe.whitelist()
def check_username_vdom_exists(username, vdom, exclude_name=None):
    filters  = {"custom_dfc_1_username": username, "custom_virtual_domain": vdom or "root"}
    existing = frappe.db.get_value("DFC 1 User", filters, "name")
    if not existing:
        return False
    if exclude_name and existing == exclude_name:
...
def get_vdoms():
    ...
    try:
        ...
        frappe.log_error(str(e), "FortiGate Get VDOMs Error DFC1 User")
...
def create_fortigate_user(docname, password=None):
    doc         = frappe.get_doc("DFC 1 User", docname)
    vdom        = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    fg_username = doc.custom_dfc_1_username
    try:
        plain_password = password or get_decrypted_password("DFC 1 User", docname, "password")
    except Exception:
...
# -------------------------------------------------------
# User Group CRUD (placeholder — to be expanded later)
# -------------------------------------------------------
@frappe.whitelist()
def create_fortigate_user_group(docname):
    doc  = frappe.get_doc("DFC 1 User Group", docname)
    vdom = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    url  = f"{BASE_URL}/user/group?vdom={vdom}"
...
def sync_users_from_fortigate():
    try:
        ...
        for vdom in vdom_list:
            ...
            for u in users:
                ...
                existing_name = frappe.db.get_value(
                    "DFC 1 User",
                    {"custom_dfc_1_username": fg_username, "custom_virtual_domain": vdom},
                    "name"
                )

                if existing_name:
                    doc = frappe.get_doc("DFC 1 User", existing_name)
                    ...
                    try:
                        doc = frappe.get_doc({
                            "doctype":               "DFC 1 User",
                            "custom_dfc_1_username": fg_username,
                            "password":              "",
        ...
        frappe.log_error(str(e), "Fortigate DFC1 User Sync Error")
        return {"status": "error", "message": str(e)}


# -------------------------------------------------------
# Sync User Groups from FortiGate (placeholder)
# -------------------------------------------------------
@frappe.whitelist()
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for g in groups:
                ...
                if existing:
                    doc = frappe.get_doc("DFC 1 User Group", existing)
                    ...
                    for m in members:
                        ...
                        frappe_user_name = frappe.db.get_value(
                            "DFC 1 User",
                            {"custom_dfc_1_username": uname, "custom_virtual_domain": vdom},
                            "name"
                        )
                    ...
                    try:
                        ...
                        frappe.log_error(
                            ...
                            "Fortigate DFC1 User Group Sync Skip"
                        )
                        skipped += 1
        ...
        frappe.log_error(str(e), "Fortigate DFC1 User Group Sync Error")
        return {"status": "error", "message": str(e)}
...
def search_by_user_group(search_term):
    results = frappe.db.sql("""
        SELECT name, custom_dfc_1_username, user_group, custom_virtual_domain, user_account_status
        FROM `tabDFC 1 User`
...
def rename_fortigate_user(docname, new_username, ticket_id=None, remarks=None):
    doc          = frappe.get_doc("DFC 1 User", docname)
    vdom         = doc.custom_virtual_domain if doc.custom_virtual_domain else "root"
    old_username = doc.custom_dfc_1_username
    ...
    try:
        plain_password = get_decrypted_password("DFC 1 User", docname, "password")
    except Exception:
        plain_password = doc.password
    ...
    doc.custom_dfc_1_username = new_username
    doc.flags.ignore_fortigate_sync = True
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 1 User", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment",
            "comment_type": "Info",
            "reference_doctype": "DFC 1 User",
...
def rename_fortigate_user_group(docname, new_group_name, ticket_id=None, remarks=None):
    ...
    doc.save(ignore_permissions=True)
    frappe.rename_doc("DFC 1 User Group", docname, new_doc_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype": "Comment",
            "comment_type": "Info",
            "reference_doctype": "DFC 1 User Group",
            "reference_name": new_doc_name,
            "content": (
