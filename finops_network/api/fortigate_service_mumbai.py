...
def get_vdom_list_for_service():
    try:
        ...
        frappe.log_error(str(e), "VDOM Fetch Error Mumbai Service")
...
def create_service_in_firewall(service_name, protocol, low, high, vdom=None):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Service Error Mumbai")
        frappe.throw(str(e))
...
def sync_services_from_fortigate():
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            if response.status_code != 200:
                frappe.log_error(response.text, f"Mumbai Service Fetch Error - {vdom}")
                continue
            ...
            for svc in response.json().get("results", []):
                ...
                exists = frappe.db.get_value("DFC 1 Mumbai Service", {"name": doc_name}, "name")

                if exists:
                    current = frappe.db.get_value(
                        "DFC 1 Mumbai Service",
                        {"name": doc_name},
                    ...
                    if changed:
                        frappe.db.sql("""
                            UPDATE `tabDFC 1 Mumbai Service`
                            SET service_name = %s,
                                destination_port = %s,
                                low = %s,
                                high = %s,
                    ...
                    try:
                        frappe.db.sql("""
                            INSERT INTO `tabDFC 1 Mumbai Service`
                        ...
                        frappe.log_error(str(e), f"Mumbai Service Insert Failed: {doc_name}")
                        skipped += 1
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total":   frappe.db.count("DFC 1 Mumbai Service")
        }

    except Exception as e:
        frappe.log_error(str(e), "Fortigate Mumbai Service Sync Error")
        return {"status": "error", "message": str(e)}
