...
def delete_virtual_ip_on_fortigate(name, vdom):
    import urllib.parse
    url = f"https://{FIREWALL_IP}/api/v2/cmdb/firewall/vip/{urllib.parse.quote(name, safe='')}?vdom={vdom}"
    try:
        ...
        frappe.log_error(str(e), "Fortigate VIP Delete Error Bangalore")
        return False, str(e)
...
def get_vdoms():
    ...
    try:
        ...
        frappe.log_error(str(e), "FortiGate Get VDOMs Error Bangalore VIP")
...
def create_virtual_ip(docname):
    doc  = frappe.get_doc("DFC 1 Bangalore Virtual IP", docname)
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate VIP Create Error Bangalore")
        return {"status": "error", "message": str(e)}
...
def update_virtual_ip(docname):
    ...
    if doc.port_forwarding and port_mapping_normalized == "many to many":
        ...
        try:
            ...
            frappe.log_error(str(e), "VIP Update Recreate Error Bangalore")
            return {"status": "error", "message": str(e)}

    # Normal PUT update
    update_url = f"https://{FIREWALL_IP}/api/v2/cmdb/firewall/vip/{urllib.parse.quote(doc.name1, safe='')}?vdom={vdom}"
    try:
        ...
        frappe.log_error(str(e), "VIP Update Error Bangalore")
        return {"status": "error", "message": str(e)}
...
def sync_virtual_ips_from_fortigate():
    try:
        ...
        for vdom in vdoms:
            ...
            for v in vips:
                ...
                existing = frappe.db.exists(
                    "DFC 1 Bangalore Virtual IP",
                    {"name1": name, "custom_virtual_domain": vdom}
                )

                if existing:
                    doc = frappe.get_doc("DFC 1 Bangalore Virtual IP", existing)
                    ...
                    try:
                        frappe.get_doc({
                            "doctype":                  "DFC 1 Bangalore Virtual IP",
                            "name1":                    name,
        ...
        frappe.log_error(str(e), "Fortigate VIP Sync Error Bangalore")
        return {"status": "error", "message": str(e)}
...
def rename_virtual_ip(docname, new_name, ticket_id=None, remarks=None):
    ...
    frappe.db.set_value("DFC 1 Bangalore Virtual IP", docname, "name1", new_name)
    frappe.db.commit()

    frappe.rename_doc("DFC 1 Bangalore Virtual IP", docname, new_name, force=True)

    if ticket_id:
        frappe.get_doc({
            "doctype":           "Comment",
            "comment_type":      "Info",
            "reference_doctype": "DFC 1 Bangalore Virtual IP",
