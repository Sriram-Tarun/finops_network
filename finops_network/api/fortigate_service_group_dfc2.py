...
def get_vdom_list():
    try:
        ...
        frappe.log_error(str(e), "VDOM Fetch Error DFC2 Service Group")
        return {"status": "error", "message": str(e)}


@frappe.whitelist()
def create_service_group(docname):
    doc  = frappe.get_doc("DFC 2 Service Group", docname)
    ...
    try:
        ...
        if result.get("status") != "success":
            cli_error = str(result.get("cli_error", ""))
            error_code = result.get("error")
            member_names = [m["name"] for m in members]
            if (
                "not found in datasource" in cli_error
                or "value parse error" in cli_error
                or error_code == -5
            ):
                frappe.throw(
                    f"One or more services do not exist in FortiGate under VDOM '<b>{vdom}</b>':<br>"
                    f"<b>{', '.join(member_names)}</b><br><br>"
                    f"Please ensure these services exist in FortiGate under the correct VDOM before creating the group."
                )
            frappe.throw(f"FortiGate Error (HTTP {result.get('http_status','?')}): {cli_error or str(result)}")

        frappe.db.set_value("DFC 2 Service Group", docname, "custom_firewall_created", 1)
        frappe.db.commit()

        return f"Service Group '{doc.group_name.strip()}' created in FortiGate (VDOM: {vdom})"

    except Exception as e:
        frappe.log_error(str(e)[:200], "SvcGrp Create Error DFC2")
        frappe.throw(str(e))
...
def update_service_group(docname):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Update Service Group Error DFC2")
        frappe.throw(str(e))
...
def sync_service_groups_from_fortigate():
    headers     = get_headers()
    child_table = "tabService Group Member DFC 2"
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            for grp in response.json().get("results", []):
                ...
                if doc_name:
                    ...
                    if frappe.db.exists("DFC 2 Service Group", new_doc_name):
                        new_doc_name = f"{group_name}-{vdom}"

                    frappe.db.sql("""
                        INSERT INTO `tabDFC 2 Service Group`
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total":   frappe.db.count("DFC 2 Service Group")
        }

    except Exception as e:
        frappe.log_error(str(e), "Sync Service Group Error DFC2")
        return {"status": "error", "message": str(e)}
...
def get_services_by_vdom(vdom):
    ...
    try:
        ...
        return {"status": "success", "services": sorted(set(services))}

    except Exception as e:
        frappe.log_error(str(e), "Get Services By VDOM Error DFC2")
        return {"status": "error", "message": str(e)}
...
def search_services_by_vdom(doctype, txt, searchfield, start, page_len, filters):
    ...
    matched = [
        [s] for s in services
        if txt in s.lower() and s.strip().upper() != "ALL"
    ]
...
def validate_service_group_uniqueness(docname, group_name, vdom):
    ...
    if existing and existing != docname:
        return {
            "status":  "duplicate",
            "message": f"Service Group '{group_name}' already exists in VDOM '{vdom}'"
        }
    return {"status": "ok"}
