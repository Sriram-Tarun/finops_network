...
def get_vdom_list_for_service():
    try:
        ...
        frappe.log_error(str(e), "VDOM Fetch Error DFC2 Service")
...
def create_service_in_firewall(service_name, protocol, low, high, vdom=None):
    ...
    try:
        ...
        frappe.log_error(str(e), "Fortigate Service Create Error DFC2")
        frappe.throw(str(e))
...
def sync_services_from_fortigate():
    ...
    try:
        ...
        for vdom in vdom_list:
            ...
            if response.status_code != 200:
                frappe.log_error(response.text, f"DFC2 Service Fetch Error - {vdom}")
                continue
            ...
            for svc in response.json().get("results", []):
                ...
                exists = frappe.db.get_value("DFC 2 Service", {"name": doc_name}, "name")
                ...
                if exists:
                    ...
                    try:
                        ...
                        frappe.log_error(str(e), f"DFC2 Service Insert Failed: {doc_name}")
                        skipped += 1
        ...
        return {
            ...
            "created": created,
            "updated": updated,
            "total":   frappe.db.count("DFC 2 Service")
        }

    except Exception as e:
        frappe.log_error(str(e), "Fortigate Service Sync Error DFC2")
        return {"status": "error", "message": str(e)}
